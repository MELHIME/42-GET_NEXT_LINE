...
/*   Updated: 2024/04/05 21:36:16 by mel-hime         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*building[OPEN_MAX];

	if (fd < 0 || BUFFER_SIZE <= 0 || fd > OPEN_MAX)
		return (NULL);
	building[fd] = get_line_extra(building[fd], fd);
	if (!building[fd])
		return (NULL);
	line = get_perfect_line(building[fd]);
	building[fd] = get_chyata(building[fd]);
	if (ft_strlen(line) == 0)
	{
		free (building[fd]);
		building[fd] = NULL;
